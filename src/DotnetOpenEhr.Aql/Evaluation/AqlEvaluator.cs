...
public sealed class AqlEvaluator
{
        ...
        RegexEvaluationContext regexContext)
    {
        ...
        {
            current = PathNavigator.FilterByPredicate(current, path.Steps[0].NodeIdPredicate, null);
            startIdx = 1;
        }
        ...
        for (int i = startIdx; i < path.Steps.Count; i++)
        {
            ...
            current = PathNavigator.GetAttribute(current, step.AttributeName);
            if (step.NodeIdPredicate is not null && current is not null)
            {
                current = PathNavigator.FilterByPredicate(current, step.NodeIdPredicate, null);
            }
        }
